...
//                //$item[$this->getData('name')] = $customer->getAvatar();
//                //$customer = new \Magento\Framework\DataObject($item);
//
//                $picture_url = !empty($customer->getAvatar()) ? $this->urlBuilder->getUrl('pub/media'.$customer->getAvatar()) : $this->viewFileUrl->getUrl('Magenest_Movie::images/no-profile-photo.jpg');
//
//                $item[$fieldName . '_src'] = $picture_url;
//                $item[$fieldName . '_orig_src'] = $picture_url;
//                $item[$fieldName . '_alt'] = 'The avatar';
//            }
//        }
//
//        return $dataSource;
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                $picture_url = !empty($customer["profile_picture"]) ? $this->urlBuilder->getUrl(
                    'customer/index/viewfile/image/' . base64_encode($customer["profile_picture"])) : $this->viewFileUrl->getUrl('Magenest_Movie::images/no-profile-photo.jpg');
                $item[$fieldName . '_src'] = $picture_url;
                $item[$fieldName . '_orig_src'] = $picture_url;
